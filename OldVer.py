...
class TFM:
    ...
    def randomDoublePack(self, d1, d2, l, l1):
        a = [[random.randint(d1, d2) for j in range(l1)] for i in range(l)]
        return a

    def swipeStr(self, matrix, n, m):
        matrix[n], matrix[m] = matrix[m], matrix[n]

    def takeColumn(self, matrix, n):
...
sorting_tools = ST()
matrix_tools = TFM()
array = matrix_tools.randomDoublePack(0, 1, 5, 5)
matrix_tools.show(array)
